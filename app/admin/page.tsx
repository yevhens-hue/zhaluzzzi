...
export default function AdminPage() {
  ...
  // Active subtab
  const [activeTab, setActiveTab] = useState<'orders' | 'leads' | 'products' | 'filters' | 'calculator' | 'gallery' | 'contacts' | 'promo' | 'logs' | 'db'>('orders');
  ...
  // Add new filter items temp state
    ...
    <div className="space-y-6 py-6">
      ...
      <div className="flex border-b border-gray-200 gap-4 text-xs sm:text-sm font-bold overflow-x-auto pb-1">
        ...
        >
          ...
          <span>Фільтри каталогу ({filtersForm.destinations.filter((d) => d.enabled).length})</span>
        </button>
      ...
      {/* ----------------- 3.5 CATALOG FILTERS CMS ----------------- */}
      {activeTab === 'filters' && (
        <form onSubmit={handleSaveFilters} className="space-y-6 max-w-5xl">
          {/* Header Card */}
                  ...
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        <span className={`text-[10px] font-bold px-1.5 py-0.2 rounded-md ${dest.enabled !== false ? 'text-emerald-700 bg-emerald-50' : 'text-gray-500 bg-gray-200'}`}>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (!newDestKey || newDestKey === newDestName.toLowerCase().replace(/[^a-z0-9]+/g, '-')) {
                        const translit = e.target.value.toLowerCase()
                          .replace(/[а-яієїґ]/g, (ch) => ({'а':'a','б':'b','в':'v','г':'h','ґ':'g','д':'d','е':'e','є':'ye','ж':'zh','з':'z','и':'y','і':'i','ї':'yi','й':'y','к':'k','л':'l','м':'m','н':'n','о':'o','п':'p','р':'r','с':'s','т':'t','у':'u','ф':'f','х':'kh','ц':'ts','ч':'ch','ш':'sh','щ':'shch','ь':'','ю':'yu','я':'ya'}[ch] || ch))
                          .replace(/[^a-z0-9]+/g, '-')
                          .replace(/^-+|-+$/g, '');
